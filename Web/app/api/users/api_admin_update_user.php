	...
	if ($_SESSION["id_role"] == 3) {
		if (isset($data_from_client)) {
			...
			if (!$error) {
				...
				$sql_resquest_field_list = ["id_user", "username", "email", "first_name", "last_name", "birth_date", "adress", "city", "hire_date", "id_role", "zipcode"];
				$sqlr_field = [];
				...
				foreach ($sqlr_field as $value) {
					if ($value != "id_user") {
						array_push($sqlr_values_update, $value . " = :" . $value);
					}
				}
				unset($value);
